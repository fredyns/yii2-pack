...
namespace fredyns\pack\libraries;
...
class ActionControl extends \yii\base\Object
{
    ...
    {
        $this->errors[$name][] = sprintf($message, $name);
    }
     ...
     * @return array|string
     */
    public function getError($name, $implode = false)
    {
    ...
    public function allow($action, $params = [])
    {
        if (array_key_exists($action, $this->allowed) == FALSE) {
            ...
            if (method_exists($this, $function)) {
                ...
                $this->addError($action, Yii::t("pack/action-control", "%s is not configured properly."));
    ...
    public function getAllowView($params = [])
    {
        ...
        if (($this->model instanceof ActiveRecord) == FALSE) {
            $this->addError($action, Yii::t("pack/action-control", "Unknown Data."));
        ...
        if ($this->model->isNewRecord) {
            $this->addError($action, Yii::t("pack/action-control", "Can't %s unsaved data."));
        }
    ...
    public function getAllowDelete($params = [])
    {
        ...
        if ($this->model->hasAttribute('recordStatus') && $this->model->hasAttribute('deleted_at')) {
            if ($this->model->getAttribute('recordStatus') == 'deleted') {
                $this->addError($action, Yii::t("pack/action-control", "Data already (soft) deleted."));
            }
        }
    ...
    public function getAllowRestore($params = [])
    {
        ...
        if ($this->model->hasAttribute('recordStatus') == FALSE OR $this->model->hasAttribute('deleted_at') == FALSE) {
            $this->addError($action, Yii::t("pack/action-control", "Data doesn't support soft-delete."));
        } elseif ($this->model->getAttribute('recordStatus') != 'deleted') {
            $this->addError($action, Yii::t("pack/action-control", "Data is not deleted."));
        }
